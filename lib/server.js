...
const { TRASH_RETENTION_DAYS } = require('./store');
...
class Server {
    ...
    _guardApiWrite(req, res, next) {
        ...
        const tok = req.get('X-Kanban-Token') || (req.body && req.body._token) || '';
    ...
    /** Ist der Token auf bestimmte Boards begrenzt (allowedBoards != '*')? */
    _tokenScoped(entry) {
    ...
    _guardTokenScope(req, res, next) {
        ...
        const body = req.body || {};
        for (const key of ['board', 'boardId', 'toBoard', 'targetBoard']) {
        ...
        if (!targets.size) {
            ...
            this.adapter.log.warn(`Token '${entry.name || '?'}' darf ${req.method} ${req.originalUrl} nicht ausführen (auf Boards begrenzt)`);
        ...
        for (const boardId of targets) {
            if (!this._boardAllowed(entry, boardId)) {
                this.adapter.log.warn(`Token '${entry.name || '?'}' darf Board '${boardId}' nicht ändern (${req.method} ${req.originalUrl})`);
    ...
    _webhookRoutes(app) {
        ...
        router.post('/action', wrap(async (req, res) => {
            const body = req.body || {};
            // Alle Board-Angaben prüfen, nicht nur die Quelle: transferCard nimmt
            // mit toBoard/targetBoard ein zweites Board entgegen.
            for (const key of ['board', 'boardId', 'toBoard', 'targetBoard']) {
                if (body[key] && !guardBoard(req, res, String(body[key]))) return;
            }
            const source = `webhook:${req.tokenEntry.name || 'token'}`;
