...
class Server {
    ...
    _apiRoutes(app) {
        ...
        // ---- Diagnose: Ablauf eines Zuges auf Touch-Geraeten ----------------
        // Voruebergehend, um eine Regression einzukreisen: Das Board schickt am
        // Ende eines Zuges die Ereignisfolge her, hier liegen die letzten paar
        // im Speicher und lassen sich per GET abholen. Damit kommt man auch an
        // ein Tablet im Kiosk-Modus heran, wo weder URL noch Konsole erreichbar
        // sind. Das POST laeuft ueber denselben Token-Schutz wie jeder andere
        // Schreibzugriff, es entsteht also kein offener Endpunkt.
        app.post('/api/debug/drag', (req, res) => {
            const body = req.body || {};
            this._dragTraces = (this._dragTraces || []).slice(-9);
            this._dragTraces.push({
                at: new Date().toISOString(),
                reason: String(body.reason || '').slice(0, 40),
                ua: String(body.ua || '').slice(0, 200),
                screen: String(body.screen || '').slice(0, 20),
                coarse: !!body.coarse,
                trace: (Array.isArray(body.trace) ? body.trace : []).slice(0, 60).map(s => String(s).slice(0, 120)),
            });
            res.json({ ok: true });
        });

        app.get('/api/debug/drag', (req, res) => res.json(this._dragTraces || []));
