...
class Server {
    ...
    _tz() {
        return this.adapter._timezone || 'Europe/Berlin';
    }
    _pubCard(card) {
        return cardWithDueAt(card, this._tz());
    }
    _pubBoard(board) {
        return boardWithDueAt(board, this._tz());
    }
    ...
    async start() {
        ...
        app.use(
            ...
            express.static(path.join(__dirname, '..', 'www'), {
                setHeaders: (res, filePath) => {
                    ...
                    if (/\.(html|js|mjs|json|css)$/i.test(filePath)) {
        ...
        this._pingInterval = this.adapter.setInterval(() => {
        ...
        if (port !== (cfg.port || 8095)) {
            ...
            this.adapter.log.warn(
                `Port ${cfg.port} is in use - falling back to free port ${port}. ` +
    ...
    async stop() {
        if (this._pingInterval) {
            this.adapter.clearInterval(this._pingInterval);
        }
        if (this.wss) {
            for (const ws of this.wss.clients) {
                ws.terminate();
        ...
        if (this.server) {
            // Ohne das wartet close auf jede Keep-Alive-Verbindung. Ein offener
            // Browser-Tab reicht, damit das Herunterfahren haengt und der Host
            // den Adapter nach einer Sekunde mit SIGKILL beendet (Befund 18).
    ...
    _guardApiWrite(req, res, next) {
        ...
        if (this.adapter.config.apiWriteProtection === false) {
            return next();
        }
        ...
        if (tok === this.adapter._apiSecret) {
            return next();
        }
    ...
    _findToken(value) {
        ...
        return (
            (this.adapter.config.inboundTokens || []).find(
                t => t && t.enabled !== false && String(t.token || '').trim() === tok,
            ) || null
        );
    }
    ...
    _guardTokenScope(req, res, next) {
        ...
        if (!targets.size) {
            ...
            this.adapter.log.warn(
                `Token '${entry.name || '?'}' may not perform ${req.method} ${req.originalUrl} (limited to specific boards)`,
            );
        ...
        for (const boardId of targets) {
            if (!this._boardAllowed(entry, boardId)) {
                this.adapter.log.warn(
                    `Token '${entry.name || '?'}' may not modify board '${boardId}' (${req.method} ${req.originalUrl})`,
                );
    ...
    _apiRoutes(app) {
        ...
        // Verwaiste Zustaendige: auflisten und umhaengen. Beides fasst alle Boards
        // an, steht board-begrenzten Tokens also nicht offen - dafuer sorgt die
        // Schreibpruefung, die ohne Eintrag in COMMAND_BOARDS greift.
        app.get(
            '/api/users/orphaned',
            wrap(async (req, res) => {
                res.json({ orphaned: this.store.findOrphanedAssignees() });
            }),
        );

        // Die Karten hinter einer verwaisten ID - fuer die Liste im
        // Einstellungsdialog. Getrennt vom Ueberblick oben, damit der
        // Einstiegsaufruf schlank bleibt und erst beim Aufklappen laedt.
        app.get(
            '/api/users/orphaned/:name',
            wrap(async (req, res) => {
                const limit = Number(req.query.limit) || 0;
                res.json(this.store.orphanedCards(req.params.name, limit));
            }),
        );

        app.post(
            '/api/users/:name/reassign',
            wrap(async (req, res) => {
                const to = String((req.body || {}).to || '');
                try {
                    res.json(await this.store.reassignUser(req.params.name, to, (req.body && req.body.by) || 'api'));
                } catch (e) {
        ...
        app.post(
            ...
            wrap(async (req, res) => {
                ...
                if (buf.length > 512 * 1024) {
                    return res.status(413).json({ error: 'Bild zu groß' });
                }
                await this.adapter.writeFileAsync(
                    this.adapter.namespace,
                    `avatars/${name.replace(/[^a-z0-9_-]/gi, '')}.png`,
                    buf,
                );
                res.json({ ok: true });
            }),
        ...
        app.delete(
            ...
            wrap(async (req, res) => {
                const name = String(req.params.name).replace(/[^a-z0-9_-]/gi, '');
                try {
                    await this.adapter.delFileAsync(this.adapter.namespace, `avatars/${name}.png`);
                } catch {
                    /* egal */
                }
                res.json({ ok: true });
            ...
            wrap(async (req, res) => {
                ...
                if (body.color !== undefined) {
                    ...
                    if (!/^#[0-9a-fA-F]{3,8}$/.test(c)) {
                        return res.status(400).json({ error: 'Ungueltige Farbe' });
                    }
                    colors[name] = c;
                }
        ...
        app.get(
            ...
            wrap(async (req, res) => {
                ...
                if (!board) {
                    return res.status(404).json({ error: `Board '${req.params.id}' existiert nicht` });
                }
        ...
        app.post(
            ...
            wrap(async (req, res) => {
                ...
                res.json(
                    this._pubCard(this.store.moveCard(req.params.id, req.params.cardId, columnId, order, by || 'api')),
                );
            }),
        );
        ...
        app.post(
            '/api/boards/:id/cards/:cardId/restore',
            wrap(async (req, res) => {
                const { columnId, by } = req.body || {};
                res.json(
                    this._pubCard(this.store.restoreCard(req.params.id, req.params.cardId, columnId, by || 'api')),
                );
            }),
        );

        app.post(
            '/api/boards/:id/cards/:cardId/purge',
            wrap(async (req, res) => {
                res.json(
                    this._pubCard(
                        this.store.purgeCard(req.params.id, req.params.cardId, (req.body && req.body.by) || 'api'),
                    ),
                );
            }),
        );
        ...
        app.post(
            '/api/boards/:id/cards/:cardId/transfer',
            wrap(async (req, res) => {
    ...
    _boardAllowed(entry, boardId) {
        ...
        if (!s) {
            return false;
        } // leere Liste = kein Board, nicht alle
    ...
    checkActionCommand(entry, body) {
        ...
        const targets = [];
        if (spec.source) {
            targets.push(body.board || body.boardId);
        }
        if (spec.target) {
            targets.push(body.toBoard || body.targetBoard);
        }
    ...
    async _avatarExists(name) {
        ...
        try {
            return !!(await this.adapter.fileExistsAsync(this.adapter.namespace, `avatars/${n}.png`));
        } catch {
            return false;
        }
    }
    ...
    async _writeUserColors(map) {
        await this.adapter.writeFileAsync(
            this.adapter.namespace,
            'usercolors.json',
            Buffer.from(JSON.stringify(map), 'utf8'),
        );
    }
    ...
    _webhookRoutes(app) {
        ...
        router.post(
            '/boards/:id/cards',
            wrap(async (req, res) => {
                if (!guardBoard(req, res, req.params.id)) {
                    return;
                }
        ...
        router.post(
            ...
            wrap(async (req, res) => {
                ...
                if (denied) {
                    this.adapter.log.warn(
                        `Token '${req.tokenEntry.name || '?'}' denied for command '${body.cmd}': ${denied.error}`,
                    );
