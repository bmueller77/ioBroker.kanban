...
function cronRrule(expr) {
    ...
    if (domAll && dowAll && monAll) {
        return 'RRULE:FREQ=DAILY';
    }
    if (domAll && monAll && !dowAll) {
        // Cron kennt Sonntag als 0 und als 7, ICS_DAYS ist mit 1..7 (Mo..So) indiziert:
        // ein blosses v % 7 macht aus beiden die 0 und liess den Sonntag herausfallen.
        const d = list(dow, v => ICS_DAYS[v === 0 ? 7 : v]);
        const uniq = d && [...new Set(d)]; // '0,7' nennt denselben Tag zweimal
        return uniq && uniq.length ? `RRULE:FREQ=WEEKLY;BYDAY=${uniq.join(',')}` : '';
    }
...
function buildRrule(rec) {
    ...
    switch (rec.type) {
        case 'daily':
            return 'RRULE:FREQ=DAILY';
        case 'every_n_days':
            return `RRULE:FREQ=DAILY;INTERVAL=${Math.max(1, Number(rec.interval) || 1)}`;
        case 'weekly':
            return days.length ? `RRULE:FREQ=WEEKLY;BYDAY=${days.join(',')}` : 'RRULE:FREQ=WEEKLY';
        case 'monthly':
            return `RRULE:FREQ=MONTHLY;BYMONTHDAY=${Math.min(31, Math.max(1, Number(rec.dayOfMonth) || 1))}`;
        ...
        case 'cron':
            return cronRrule(rec.cron);
        default:
            // workday und every_n_days_done sind nicht als Serie abbildbar.
            // Beim zweiten liegt es in der Natur der Sache: Wann der naechste
            // Termin faellt, entscheidet erst der Haken. Eine feste Regel im
            // Kalender waere also eine Behauptung, die niemand einhaelt.
            // Stattdessen bleibt es bei einem Einzeltermin.
            return '';
    }
}
 ...
 */
/**
 * Absage zu einer bereits verschickten Einladung.
 *
 * Dieselbe UID, eine hoehere Sequenznummer und METHOD:CANCEL: Daran erkennt
 * jeder Kalender den vorhandenen Eintrag und raeumt ihn ab. Ohne das blieb der
 * Termin nach dem Loeschen der Karte bei allen Zustaendigen stehen (E6).
 *
 * @param card die geloeschte Karte
 * @returns .ics-Inhalt
 */
function buildIcsCancel(card) {
    return [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//ioBroker//Kanban//DE',
        'CALSCALE:GREGORIAN',
        'METHOD:CANCEL',
        'BEGIN:VEVENT',
        `UID:${card.icsUid || card.id}@kanban.iobroker`,
        `SEQUENCE:${(Number(card.icsSeq) || 0) + 1}`,
...
function buildIcs(card, tz) {
    ...
    if (card.description) {
        lines.push(`DESCRIPTION:${icsEsc(card.description)}`);
    }
    if (card.location) {
        lines.push(`LOCATION:${icsEsc(card.location)}`);
    }
    if (card.link) {
        lines.push(`URL:${icsEsc(card.link)}`);
    }
...
class Notifier {
    constructor(adapter, getBaseUrl, getTimezone) {
        ...
        // Welche Einladung zuletzt zu welcher Karte rausging. Der Store merkt
        // sich das auch, aber beide Ereignisse einer neu angelegten Karte
        // tragen einen Schnappschuss von vor diesem Vermerk - dann haengen
        // beide die .ics an (E5).
        this._icsSent = new Map();
    }
    ...
    async _dispatch(event) {
        ...
        if (event && event.board && event.board.id && event.card && event.card.id) {
            ...
            if (base) {
                event.link = `${base}/?board=${encodeURIComponent(event.board.id)}&card=${encodeURIComponent(event.card.id)}`;
            }
        }
        // Fälligkeit inkl. Uhrzeit als kombiniertes Feld (lokaler Offset) mitgeben
        if (event && event.card) {
            event.dueAt = buildDueAt(event.card, this.getTimezone && this.getTimezone());
        }
        await this.adapter.setStateAsync('lastEvent', JSON.stringify(event), true);
        await Promise.allSettled([this._sendEmails(event), this._sendWebhooks(event)]);
    }
    ...
    _userWants(u, flag) {
        ...
        if (v === undefined || v === null || v === '') {
            return !!this.adapter.config[flag];
        }
        return !!v;
    }
    ...
    _icsDecision(event, card) {
        if (!card || !card.calendarInvite || !card.due) {
            return null;
        }
        if (event.detail && event.detail.recurrence) {
            return null;
        } // Folgekarte einer Serie
        const fp = icsFingerprint(card);
        // Schon in diesem Atemzug verschickt: "Karte angelegt" und "Karte
        // zugewiesen" feuern beide beim Anlegen (E5).
        if (this._icsSent.get(card.id) === fp) {
            return null;
        }
        const changed = fp !== (card.icsFingerprint || '');
        const firstContact = event.event === 'cardCreated' || event.event === 'cardAssigned';
        if (!changed && !firstContact) {
            return null;
        }
        ...
        if (store && store.markIcsSent && event.board) {
            try {
                store.markIcsSent(event.board.id, card.id, fp, seq);
            } catch {
                /* ignore */
            }
        }
        this._icsSent.set(card.id, fp);
        // Nicht unbegrenzt wachsen lassen: Der Eintrag wird nur gebraucht,
        // solange die Ereignisse einer Aenderung nacheinander eintreffen.
        if (this._icsSent.size > 500) {
            this._icsSent.delete(this._icsSent.keys().next().value);
        }
        return { seq };
    }
    ...
    _recipientUsers(event) {
        ...
        let names;
        if (event.event === 'cardAssigned') {
            names = [event.detail && event.detail.assignee].filter(Boolean);
        } else if (event.event === 'cardCreated') {
            names = this._boardMembers(event);
        } else {
            names = (event.card && event.card.assignees) || [];
        }
        const seen = new Set();
        const users = [];
    ...
    async _sendEmails(event) {
        ...
        let attachments;
        // Wird die Karte geloescht, geht keine Einladung mehr raus, sondern
        // eine Absage - und nur, wenn ueberhaupt je eine verschickt wurde (E6).
        const abgesagt =
            (event.event === 'cardDeleted' || event.event === 'cardPurged') &&
            card.calendarInvite &&
            card.due &&
            card.icsFingerprint;
        if (abgesagt) {
            this._icsSent.delete(card.id);
            attachments = [
                {
                    filename: 'termin.ics',
                    content: buildIcsCancel(card),
                    contentType: 'text/calendar; charset=utf-8; method=CANCEL',
                },
            ];
        }
        const icsInfo = abgesagt ? null : this._icsDecision(event, card);
        if (icsInfo) {
            attachments = [
                {
                    filename: 'termin.ics',
        ...
        this.adapter.log.debug(
            `E-mail '${subject}' to ${users.map(u => u.email).join(', ')}${attachments ? ' (+ .ics)' : ''}`,
        );
    }
    ...
    _renderHtml(event, label) {
        ...
        if (card.due) {
            rows.push([serverT(lang, 'mail.due'), esc(card.due)]);
        }
        ...
        if (event.detail && event.detail.by) {
            rows.push([serverT(lang, 'mail.by'), esc(event.detail.by)]);
        }
        if (card.description) {
            rows.push([serverT(lang, 'mail.description'), esc(card.description).slice(0, 500)]);
        }
...
<table style="border-collapse:collapse">${rows
            .map(
                ([k, v]) =>
                    `<tr><td style="padding:2px 12px 2px 0;color:#777">${k}</td><td style="padding:2px 0">${v}</td></tr>`,
            )
            .join('')}
</table>
${link}
<p style="color:#999;font-size:12px">ioBroker Kanban · ${esc(event.ts)}</p>
    ...
    async sendCleanupSummary(results) {
        const cfg = this.adapter.config;
        if (!cfg.emailInstance || !Array.isArray(results) || !results.length) {
            return;
        }
        const lang = this.adapter._language;
        ...
        const add = (u, boardTitle, kind, card) => {
            ...
            if (!e) {
                e = { user: u, boards: new Map() };
                perUser.set(u.email, e);
            }
            let b = e.boards.get(boardTitle);
            if (!b) {
                b = { moved: [], purged: [] };
                e.boards.set(boardTitle, b);
            }
            b[kind].push(card);
        };
        for (const r of results) {
            ...
            for (const card of r.moved) {
                for (const name of card.assignees || []) {
                    ...
                    if (u && this._userWants(u, 'notifyDeleted')) {
                        add(u, boardTitle, 'moved', card);
                    }
                }
            }
            for (const card of r.purged) {
                for (const name of card.assignees || []) {
                    const u = this._userByName(name);
                    if (u && this._userWants(u, 'notifyPurged')) {
                        add(u, boardTitle, 'purged', card);
                    }
                }
            }
        }
        ...
        if (perUser.size) {
            this.adapter.log.debug(`Cleanup summary to ${perUser.size} recipient(s)`);
        }
    }
    ...
    _renderCleanupHtml(boards, lang) {
        ...
        for (const [boardTitle, g] of boards) {
            ...
            if (g.moved.length) {
                out += `<p style="margin:4px 0;color:#555">${esc(serverT(lang, 'mail.cleanupIntro'))}</p><ul>${g.moved
                    .map(c => `<li>${esc(c.title)}</li>`)
                    .join('')}</ul>`;
            }
            if (g.purged.length) {
                out += `<p style="margin:4px 0;color:#555">${esc(serverT(lang, 'mail.purgedIntro'))}</p><ul>${g.purged
                    .map(c => `<li>${esc(c.title)}</li>`)
                    .join('')}</ul>`;
            }
        }
    ...
    async _sendWebhooks(event) {
        const hooks = (this.adapter.config.outboundWebhooks || []).filter(
            h => h && h.enabled !== false && h.url && this._matchesEvents(h.events, event.event),
        );
    ...
    async _post(hook, event, isRetry) {
        ...
        try {
            ...
            if (!isRetry) {
                await new Promise(r => this.adapter.setTimeout(r, 2000));
...
module.exports = { Notifier, buildIcs, buildIcsCancel, buildRrule, icsFingerprint };
