...
// nowHHMM und isOverdue kommen aus dem Store: Die Ueberfaelligkeit darf nur an
// einer Stelle definiert sein, sonst laufen Zaehler und Ereignis auseinander.
const { todayStr, nowHHMM, isOverdue } = require('./store');
        ...
        for (const board of this.store.boards.values()) {
            ...
            for (const card of board.cards) {
                ...
                this.bus.emitEvent('cardDue', {
                    board: { id: board.id, title: board.title },
                    card,
                    detail: { due: card.due, overdue: isOverdue(card, today, nowHHMM()) },
                });
            }
            if (touched) {
                board.rev++;
