 ...
 * (`*​/3`, `1-7/2`) sowie die englischen Kurznamen für Monat und Wochentag.
...
function namedValue(token, fieldName) {
    ...
    if (fieldName === 'month' && MONTH_NAMES[key] !== undefined) return MONTH_NAMES[key];
    if (fieldName === 'dayOfWeek' && DAY_NAMES[key] !== undefined) return DAY_NAMES[key];
    return null;
}
...
function parseField(raw, field) {
    ...
    if (!spec) throw new Error(`${field.name}: leer`);
    const out = new Set();

    for (const part of spec.split(',')) {
        const piece = part.trim();
        if (!piece) throw new Error(`${field.name}: leerer Listeneintrag in '${spec}'`);

        const [rangePart, stepPart] = piece.split('/');
        let step = 1;
        if (stepPart !== undefined) {
            step = Number(stepPart);
            if (!Number.isInteger(step) || step < 1) throw new Error(`${field.name}: ungültige Schrittweite '${stepPart}'`);
        }
        ...
        if (rangePart === '*') {
            ...
            if (from === null) from = Number(a);
            to = namedValue(b, field.name);
            if (to === null) to = Number(b);
        } else {
            from = namedValue(rangePart, field.name);
            if (from === null) from = Number(rangePart);
    ...
    if (field.name === 'dayOfWeek' && out.has(7)) { out.delete(7); out.add(0); }
    return out;
}
...
function parseCron(expr) {
    ...
    if (parts.length !== 5) {
        throw new Error(`Cron braucht fünf Felder (Minute Stunde Tag Monat Wochentag), gefunden: ${parts.filter(Boolean).length}`);
    }
    const parsed = {};
    FIELDS.forEach((field, i) => { parsed[field.name] = parseField(parts[i], field); });
