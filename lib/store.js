...
class Store {
    ...
    _ensureTrashColumn(board) {
        ...
        if (!tc) {
            ...
            tc = { id, title: serverT(this.adapter._language || 'en', 'col.trash'), wipLimit: 0, isDone: false, isTrash: true };
    ...
    _spawnRecurrence(board, doneCard, by) {
        ...
        const fresh = {
            ...
            movedAt: null, doneAt: null, trashedAt: null, lastReminderAt: null,
        };
    ...
    transferCard(fromBoardId, cardId, toBoardId, toColumnId, mode, overrides, source) {
        ...
        if (fromBoardId === toBoardId) throw new Error('Quell- und Ziel-Board sind identisch');
        ...
        if (mode === 'copy') {
            const copy = {
                id: genId('c'),
                columnId: target.id,
                order: to.cards.filter(c => c.columnId === target.id).length,
                ...
                doneAt: isDone ? nowIso : null, trashedAt: null, lastReminderAt: null,
            };
            this._mutate(to, b => b.cards.push(copy));
            this.bus.emitEvent('cardCreated', { board: this._boardRef(to), card: copy, detail: { by, crossBoardCopy: true, fromBoard: from.id } });
