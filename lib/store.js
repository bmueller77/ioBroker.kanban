...
function recMatches(rec, d) {
    switch (rec.type) {
        ...
        case 'cron': {
            ...
            try { return cronMatchesDate(parseCron(rec.cron), d); }
...
function assertDate(v, field) {
    ...
    const s = String(v);
    if (!DATE_RE.test(s)) throw new Error(`${field} muss im Format YYYY-MM-DD vorliegen, nicht '${s}'`);
    return s;
}
function assertTime(v, field) {
    if (v === undefined || v === null || v === '') return '';
    const s = String(v);
    if (!TIME_RE.test(s)) throw new Error(`${field} muss im Format HH:MM vorliegen, nicht '${s}'`);
    return s;
}
...
function assertRecurrence(rec) {
    if (!rec || !rec.type || rec.type === 'none') return '';
    if (rec.type !== 'cron') return '';
    const check = validateCron(rec.cron);
    if (!check.ok) throw new Error(`Cron-Ausdruck ungültig: ${check.error}`);
    return check.time;
}
function assertPriority(v) {
    if (v === undefined || v === null || v === '') return 0;
    const n = Number(v);
    if (!Number.isInteger(n) || n < 0 || n > 2) throw new Error(`priority kennt nur 0, 1 oder 2, nicht '${v}'`);
    return n;
}
...
class Store {
    ...
    _ensureTrashColumn(board) {
        ...
        for (const col of board.columns) {
            if (col !== tc && col && col.id === tc.id) {
                col.id = genId('col');
                changed = true;
                this.adapter.log.warn(`Board '${board.id}': a second column used the trash column id '${tc.id}' `
        ...
        return this._mutate(board, b => {
            ...
            if (Array.isArray(patch.columns)) {
                ...
                const normal = patch.columns.filter(c => !c.isTrash).map(c => {
                    ...
                    if (!id || used.has(id)) id = genId('col');
                    used.add(id);
                    return {
                        id,
    ...
    _logIrreversible(cmd, boardId, cardId, source) {
        this.adapter.log.info(`${cmd}${boardId ? ` on board '${boardId}'` : ''}`
    ...
    addCard(boardId, data, source) {
        ...
        const recurrence = (data.recurrence && data.recurrence.type && data.recurrence.type !== 'none') ? data.recurrence : null;
    ...
    updateCard(boardId, cardId, patch, source) {
        ...
        if (patch.due !== undefined) assertDate(patch.due, 'due');
        if (patch.dueTime !== undefined) assertTime(patch.dueTime, 'dueTime');
        if (patch.priority !== undefined) assertPriority(patch.priority);
        if (patch.recurrence !== undefined) assertRecurrence(patch.recurrence);
        this._mutate(board, () => {
    ...
    purgeCard(boardId, cardId, source, opts = {}) {
        const board = this._requireBoard(boardId);
        if (!opts.auto) this._logIrreversible('purgeCard', boardId, cardId, source);
    ...
    runCleanup(nowMs) {
        ...
        for (const board of this.boards.values()) {
            ...
            const purged = board.cards.filter(c => {
                if (c.columnId !== trash.id) return false;
                const trashedMs = Date.parse(c.trashedAt);
                if (!Number.isFinite(trashedMs)) return false;
            ...
            if (cl.mode === 'age' || cl.mode === 'count') {
                ...
                if (cl.mode === 'age') {
                    ...
                    toMove = board.cards.filter(c => {
                        if (!doneColIds.includes(c.columnId)) return false;
