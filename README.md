...
- **0.3.0**
  ...
  - **Fix: the "views" dialog kept offering the users of the board you were looking at.** Switching the board inside the dialog reloaded labels and columns but not the users, so the generated `users=` link could name people who belong to no one on the target board — and then silently filtered nothing at all
  - **Fix: copying or moving a card to a board created through the API was a dead end.** Such a board starts with an empty member list, which the server reads as "everyone" but the dialog read literally as "no one" — it demanded an assignee while offering none, leaving the OK button greyed out for good
  - **Fix: a board refresh could pull you back to the board you just left.** A reply that arrived after you had switched boards overwrote the new one, and two overlapping refreshes could write an older revision over a newer one
  ...
  - **Fix: `purgeCard` bypassed the retention period.** The call did not check whether the card was in the trash at all, so a single request could remove any active card irreversibly. It now answers `400` unless the card really is in the trash
  ...
  - The same dialog can **clone a card in place**: three buttons at the top (clone / copy / move) decide what happens, and a clone duplicates the card with all its content directly below the original
  - The **created** notification now reaches all members of the board instead of only the assignees of the card, so you also hear about cards someone else is responsible for
  ...
  - Subtle depth for the board, all lit from the top left: cards, avatars, chips, header controls and buttons cast a
  ...
  - Fix: ticking a checklist item on a card no longer scrolls the board back to the top — column, board and page scroll positions survive the re-render
