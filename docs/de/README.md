...
- **WIP-Limit** (Work-in-Progress): Zahl > 0 begrenzt die empfohlene Kartenanzahl. Wird sie überschritten, warnt die Spalte optisch (Zähler & Kopf werden hervorgehoben). `0` = kein Limit. Das Limit ist eine **Warnung**, keine harte Sperre. Sie bezieht sich immer auf die **Gesamtzahl** der Spalte, auch wenn der Personen-/Label-Filter gerade weniger Karten anzeigt. Ist es überschritten, bleibt es seit 0.3.2 auch bei aktivem Filter neben der Zahl stehen, etwa "2/3". Der Tooltip nennt dann beide Zahlen: wie viele Karten wirklich in der Spalte liegen und wie viele der Filter davon zeigt. Vorher stand dort nur die Trefferzahl, und zwei Karten bei einem Limit von drei sahen aus wie ein Fehler statt wie eine Warnung.
