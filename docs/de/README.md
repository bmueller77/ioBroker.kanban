...
- **Anzeige-Limit (Max):** Zahl > 0 zeigt in dieser Spalte nur die ersten N Karten; darunter erscheint `+X weitere`. `0` = alle anzeigen. Praktisch, damit lange Rückstände das Board nicht sprengen. Der Zähler in der Spaltenkopfzeile zählt weiterhin **alle** Karten der Spalte. Seit 0.3.2 ist `+X weitere` ein **Knopf**: Ein Klick zeigt die übrigen Karten, ein zweiter versteckt sie wieder. Das Limit am Board bleibt dabei unangetastet, die Abweichung merkt sich nur dein Browser, wie der Sortiermodus und das Augen-Symbol.
